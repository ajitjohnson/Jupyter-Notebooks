...
# I/O
gatorObjects = '/n/scratch3/users/a/ajn16/gator/data/1_6_GatorOutput.h5ad'
outputLocation = '/n/scratch3/users/a/ajn16/gator/results/'
outputName = 'test.csv'
...
# =============================================================================
    ...
    def bonafide_cells (marker, 
        ...
        if marker in expression_unet_common:
            ...
            if len(pos) < 3 or len(neg) < 3: ## CHECK!
...
# Next function
def permutation_function (permutation, adata, markers, n_cells, drop_markers):
    permutation_results = []
    # resolve prediction
    predictions = adata.uns['gatorPredict']
    permuted_data = predictions.copy()[markers]
    # Permute the rows of the copy of the data frame
    permuted_data = permuted_data.sample(frac=1, random_state=permutation)
    print (str(permutation))
    
    # process each maker independently
    final_score = []
    for col in permuted_data.columns:
      # Get the indices of the rows with 'pos' in the corresponding column of the predictions data frame
      col_indices_pos = permuted_data[permuted_data[col] == 'pos'].index
      col_indices_neg = permuted_data[permuted_data[col] == 'neg'].index
      # Select a subset of the indices based on the number of cells
      col_indices_subset = list(col_indices_pos[:n_cells])
      # remove this from the whole list
      col_indices_subset.extend(col_indices_neg)
      
      # subset the data
      idx_number = adata.obs.index.get_indexer(col_indices_subset)
      # prepare data
      raw_data = pd.DataFrame(adata.raw.X[idx_number, :], index = col_indices_subset, columns=adata.var.index)
      processed_data = pd.DataFrame(np.log1p(raw_data), index = col_indices_subset, columns=adata.var.index)
      # create the adata
      bdata = ad.AnnData(raw_data)
      bdata.obs = adata.obs.loc[col_indices_subset]
      bdata.raw = bdata
      bdata.X = processed_data
      # add the pre-processed data as a layer
      bdata.uns['probQuant'] = adata.uns['probQuant'].loc[col_indices_subset]
      
      # perform gator analysis
      bdata = gatorPredict (    gatorObject=bdata, 
                                probQuant='probQuant',
                                minAbundance=0,
                                percentiles=[1, 20, 80, 99],
                                drop_markers = drop_markers, 
      ...
      original_prediction = convert_to_binary(predictions.loc[col_indices_subset][col].values)
      permutation_prediction = convert_to_binary(bdata.uns['gatorPredict'][col].values)
      # convert them to binary
      score = [f1_score(original_prediction,permutation_prediction)]
...
def main():
    ...
    for i in [23]:
        ...
        partial_func = functools.partial(permutation_function, adata=adata, markers=markers, n_cells=i, drop_markers=drop_markers)
        
        # run function in parallel
        # Create a ThreadPoolExecutor with 4 threads
        with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
            # Get an iterator over the results of the permutation function
            # applied to each permutation index in parallel
            results = executor.map(partial_func, range(n_permutations))   
            # Iterate over the results and collect them in a list        
            permutation_results = [result for result in results]
            #convert to df
            idx = ["permutation_{}".format(i) for i in range(n_permutations)]
            scores = pd.DataFrame(permutation_results, columns=markers, index = idx)
        # consolidate all scores
        scores['cells'] = str(i)
        final_scores = pd.concat([final_scores, scores])
    ...
    # write it out
    o = outputLocation + outputName
    final_scores.to_csv(o)
