...
function processFile(input, output, file) {
	...
	rename("NucleiMinusContours");
	// gaussian blur, invert and add 1 to get a range of intensity between 0 and 1
	run("Gaussian Blur...", "sigma="+gaussian_blur_for_watershed);
	run("Invert");
	run("Add...", "value=1");
	// run watershed with objects as masks
	run("Classic Watershed", "input=[NucleiMinusContours] mask=[ObjectComponentMask] use min=0 max=1");
